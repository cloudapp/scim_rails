module ScimRails
  class ScimUsersController < ScimRails::ApplicationController
    def index
      ...
      if params[:filter].present?
        ...
        users = @company
          ...
          .where(
            "#{ScimRails.config.scim_users_model.connection.quote_column_name(query.attribute("users"))} #{query.operator} ?",
            query.parameter
          )
    ...
    def create
      ...
      ScimRails.config.scim_user_create_guard.call(@company, params) if ScimRails.config.scim_user_create_guard.is_a?(Proc)
